...
/*   Updated: 2023/11/08 11:22:01 by alicifer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_and_join(int fd, char *s_buffer)
{
	...
	while (!ft_strchr(joined_buffer, '\n') && n_bytes_read > 0)
	{
		...
		aux = s_buffer;
		s_buffer = ft_strjoin(aux, joined_buffer, n_bytes_read);
	}
	free (joined_buffer);
	if (ft_strlen(s_buffer) == 0)
...
char	*clean_nextline(char *s_buffer)
{
	...
	while (s_buffer[i] != '\n' && s_buffer[i] != '\0')
		i++;
	if (ft_strlen(s_buffer) - i == 0)
	{
		free (s_buffer);
		s_buffer = NULL;
		return (NULL);
	}
	nextline = ft_calloc((ft_strlen(s_buffer) - i + 1), sizeof(char));
	if (!nextline)
		return (NULL);
	j = 0;
	i++;
